...
function select_onchange()
	{	
		...
		window.location ="index.php?com=<?=$_GET['com']?>&act=man&id_item="+c.value;	
		return true;
	}
...
<?php
	...
	{
		$sql_huyen="select * from table_gallery_item where hienthi=1 order by stt,id desc ";
	...
	function get_num($id)
	{
		...
		$sql_huyen="select * from table_photo where hienthi=1 and com ='".$_GET['com']."' and id_cat =".$id." order by stt,id desc ";
...
<table class="blue_table">
	...
	<tr>
		...
		<th width="9%" style="width:6%;">Số lượng hình</th>
	...
	<tr bgcolor="<?php if($i%2==1)echo '#F3F3F3' ?>">
		...
		<td style="width:30%;" align="center"><a href="index.php?com=<?=$_GET['com']?>&act=edit&id_item=<?=$items[$i]['id_item']?>&id=<?=$items[$i]['id']?>" style="text-decoration:none;"><?=$items[$i]['ten_vi']?></a></td>
		<!-- <td style="width:30%;" align="center"><a href="index.php?com=<?=$_GET['com']?>&act=edit&id_item=<?=$items[$i]['id_item']?>&id=<?=$items[$i]['id']?>" style="text-decoration:none;"><?=$items[$i]['ten_en']?></a></td> -->
        <td style="width:9%;" align="center"><?php echo get_num($items[$i]['id'])?></td>
        <td style="width:6%;">
		<?php if(@$items[$i]['noibat']==1) { ?>
        <a href="index.php?com=<?=$_GET['com']?>&act=man&noibat=<?=$items[$i]['id']?>"><img src="media/images/active_1.png"  border="0"/></a>
        <? } else { ?>
        <a href="index.php?com=<?=$_GET['com']?>&act=man&noibat=<?=$items[$i]['id']?>"><img src="media/images/active_0.png" border="0" /></a>
        <?php } ?>        
        </td>
