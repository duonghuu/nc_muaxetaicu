...
<form name="frm" method="post" action="index.php?com=<?=$_GET['com']?>&act=save" enctype="multipart/form-data" class="form-horizontal " role="form" > 
  ...
  <div class="row">
	...
	<?php
		...
		foreach($config['lang'] as $k=>$v){ $required=''; ?>
			
		<?php $act = ''; if($k==0){ $act = 'active'; $required='required';}?>
		...
		<div class="form-group">
			...
			<div class="col-sm-10">
			  <input type="text" name="ten_<?=$v?>" value="<?=@$item['ten_'.$v]?>" class="form-control " <?=$required?> id="inputEmail3">
			</div>
		 </div>
