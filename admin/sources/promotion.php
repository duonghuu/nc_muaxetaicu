...
@define('_table','promotion');
switch($act){

	case "man":
		get_items();
		
		$template = "promotion/items";
		break;
	case "add":
		initAdd();
		$template = "promotion/item_add";
		break;
	case "edit":		
		get_item();	
		$template = "promotion/item_add";
...
function get_items(){
	...
	$sql_sp = "SELECT id,noibat FROM table_promotion where id='".$id_up."' ";
	$d->query($sql_sp);
	$cats= $d->result_array();
	$spdc1=$cats[0]['noibat'];
	//echo "id:". $spdc1;
	if($spdc1==0)
	{
	$sqlUPDATE_ORDER = "UPDATE table_promotion SET noibat ='".$noibat."' WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	}
	else
	{
	$sqlUPDATE_ORDER = "UPDATE table_promotion SET noibat =0  WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	...
	$sql_sp = "SELECT id,hienthi FROM table_promotion where id='".$id_up."' ";
	$d->query($sql_sp);
	$cats= $d->result_array();
	...
	if($hienthi==0)
	{
	$sqlUPDATE_ORDER = "UPDATE table_promotion SET hienthi =1 WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	}
	else
	{
	$sqlUPDATE_ORDER = "UPDATE table_promotion SET hienthi =0  WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	...
	if($hienthi==0)
	{
	$sqlUPDATE_ORDER = "UPDATE table_promotion SET noibat =1 WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	}
	else
	{
...
function get_item(){
	...
	$sql = "select * from #_promotion where id='".$id."'";
	$d->query($sql);
...
function delete_item(){
	...
	if(isset($_GET['id'])){
		...
		if($d->num_rows()>0){
			...
			$sql = "delete from #_promotion where id='".$id."'";
			$d->query($sql);
		}
...
function initAddRss(){
	...
	if(isAjaxRequest()){
		...
		if($_GET['method'] == 'get-item'){
			...
			$img = $_POST['image'];
			$url = $_POST['url'];
			$d->query("select * from #_promotion where ten_vi = '".$name."'");
			$data_ = array();
			if($d->num_rows() == 1){
				$data_['cls'] = 'red';
