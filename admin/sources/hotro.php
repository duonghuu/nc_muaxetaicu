...
switch($act){
	case "rss":
		...
		$template = "hotro/item_add_rss";
		break;
	case "man":
		get_items();
		
		$template = "hotro/items";
		break;
	case "add":
		initAdd();
		$template = "hotro/item_add";
		break;
	case "edit":		
		get_item();	
		$template = "hotro/item_add";
...
function get_items(){
	...
	$sql_sp = "SELECT id,tinnoibat FROM table_hotro where id='".$id_up."' ";
	$d->query($sql_sp);
	$cats= $d->result_array();
	...
	if($spdc1==0)
	{
	$sqlUPDATE_ORDER = "UPDATE table_hotro SET tinnoibat ='".$tinnoibat."' WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	}
	else
	{
	$sqlUPDATE_ORDER = "UPDATE table_hotro SET tinnoibat =0  WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	...
	$sql_sp = "SELECT id,hienthi FROM table_hotro where id='".$id_up."' ";
	$d->query($sql_sp);
	$cats= $d->result_array();
	...
	if($hienthi==0)
	{
	$sqlUPDATE_ORDER = "UPDATE table_hotro SET hienthi =1 WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	}
	else
	{
	$sqlUPDATE_ORDER = "UPDATE table_hotro SET hienthi =0  WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	...
	$sql_sp = "SELECT id,noibat FROM table_hotro where id='".$id_up."' ";
	$d->query($sql_sp);
	$cats= $d->result_array();
	...
	if($hienthi==0)
	{
	$sqlUPDATE_ORDER = "UPDATE table_hotro SET noibat =1 WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	}
	else
	{
	$sqlUPDATE_ORDER = "UPDATE table_hotro SET noibat =0  WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
...
function get_cats(){
	global $d, $items, $paging;
	$sql = "select * from #_hotro_item order by stt";
	$d->query($sql);
	$items = $d->result_array();
...
function get_cat(){
	...
	$sql = "select * from #_hotro_item where id='".$id."'";
	$d->query($sql);
...
function delete_cat(){
	...
	if(isset($_GET['id'])){
		$id =  themdau($_GET['id']);
		$d->reset();		
		$sql = "delete from #_hotro_item where id='".$id."'";
		if($d->query($sql))
			redirect("index.php?com=".$_GET['com']."&act=man_cat");
		else
...
function initAddRss(){
	...
	if(isAjaxRequest()){
		...
		if($_GET['method'] == 'get-item'){
			...
			$img = $_POST['image'];
			$url = $_POST['url'];
			$d->query("select * from #_hotro where ten_vi = '".$name."'");
			$data_ = array();
			if($d->num_rows() == 1){
				$data_['cls'] = 'red';
