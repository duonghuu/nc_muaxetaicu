...
@define("_table","recruitment");
switch($act){
	case "rss":
		initAddRss();
		
		$template = "recruitment/item_add_rss";
		break;
	case "man":
		get_items();
		
		$template = "recruitment/items";
		break;
	case "add":
		initAdd();
		$template = "recruitment/item_add";
		break;
	case "edit":	
		initAdd();		
		get_item();	
	...
	case "man_danhmuc":
		get_danhmucs();
		$template = "recruitment/danhmucs";
		break;
	case "add_danhmuc":
		initAdd("recruitment_danhmuc");
		$template = "recruitment/danhmuc_add";
		break;
	case "edit_danhmuc":
		get_danhmuc();
		$template = "recruitment/danhmuc_add";
	...
	case "man_list":
		get_lists();
		$template = "recruitment/lists";
		break;
	case "add_list":
		initAdd("recruitment_list");
		$template = "recruitment/list_add";
		break;
	case "edit_list":
		get_list();
		$template = "recruitment/list_add";
...
function get_items(){
	...
	$sql_sp = "SELECT id,tinnoibat FROM table_recruitment where id='".$id_up."' ";
	$d->query($sql_sp);
	...
	if($spdc1==0)
	{
	$sqlUPDATE_ORDER = "UPDATE table_recruitment SET tinnoibat ='".$tinnoibat."' WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	}
	else
	{
	$sqlUPDATE_ORDER = "UPDATE table_recruitment SET tinnoibat =0  WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	...
	$sql_sp = "SELECT id,hienthi FROM table_recruitment where id='".$id_up."' ";
	$d->query($sql_sp);
	$danhmucs= $d->result_array();
	...
	if($hienthi==0)
	{
	$sqlUPDATE_ORDER = "UPDATE table_recruitment SET hienthi =1 WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	}
	else
	{
	$sqlUPDATE_ORDER = "UPDATE table_recruitment SET hienthi =0  WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	...
	$sql_sp = "SELECT id,noibat FROM table_recruitment where id='".$id_up."' ";
	$d->query($sql_sp);
	$danhmucs= $d->result_array();
	...
	if($hienthi==0)
	{
	$sqlUPDATE_ORDER = "UPDATE table_recruitment SET noibat =1 WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	}
	else
	{
	$sqlUPDATE_ORDER = "UPDATE table_recruitment SET noibat =0  WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");

	}	
	}
	$sql = "select * from #_recruitment";
...
function get_item(){
	...
	$sql = "select * from #_recruitment where id='".$id."'";
	$d->query($sql);
...
function delete_item(){
	...
	if(isset($_GET['id'])){
		...
		if($d->num_rows()>0){
			...
			$sql = "delete from #_recruitment where id='".$id."'";
			$d->query($sql);
		}
...
function get_danhmucs(){
	global $d, $items, $paging;
	$sql = "select * from #_recruitment_danhmuc order by stt desc";
	$d->query($sql);
	$items = $d->result_array();
...
function get_danhmuc(){
	...
	$sql = "select * from #_recruitment_danhmuc where id='".$id."'";
	$d->query($sql);
...
function save_danhmuc(){
	...
	if($id){	
		if($photo = upload_image("file", 'jpg|png|gif|JPG|jpeg|JPEG|Jpg|PNG',_upload_tintuc,$file_name)){
			...
			$d->setTable('recruitment_danhmuc');
			$d->setWhere('id', $id);
			$d->select();
			if($d->num_rows()>0){
				$row = $d->fetch_array();
...
function delete_danhmuc(){
	...
	if(isset($_GET['id'])){
		$id =  themdau($_GET['id']);
		$d->reset();		
		$sql = "delete from #_recruitment_danhmuc where id='".$id."'";
		if($d->query($sql)){
			$d->query("select * from #_recruitment where id_danhmuc = ".$id);
			foreach($d->result_array() as $k=>$v){
				delete_file(_upload_tintuc.$v['photo']);
				delete_file(_upload_tintuc.$v['thumb']);
				$d->query("delete from #_recruitment where id=".$v['id']);
			}
			redirect("index.php?com=".$_GET['com']."&act=man_danhmuc");
		}
		else
...
function get_lists(){
	global $d, $items, $paging;
	$sql = "select * from #_recruitment_list order by stt desc";
	$d->query($sql);
	$items = $d->result_array();
...
function get_list(){
	...
	$sql = "select * from #_recruitment_list where id='".$id."'";
	$d->query($sql);
...
function delete_list(){
	...
	if(isset($_GET['id'])){
		$id =  themdau($_GET['id']);
		$d->reset();		
		$sql = "delete from #_recruitment_list where id='".$id."'";
		if($d->query($sql)){
			$d->query("select * from #_recruitment where id_list = ".$id);
			foreach($d->result_array() as $k=>$v){
				delete_file(_upload_tintuc.$v['photo']);
				delete_file(_upload_tintuc.$v['thumb']);
				$d->query("delete from #_recruitment where id=".$v['id']);
