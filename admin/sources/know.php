...
switch($act){
	...
	case "man_danhmuc":
		get_danhmucs();
		$template = "know/danhmucs";
		break;
	case "add_danhmuc":
		initAdd("know_danhmuc");
		$template = "know/danhmuc_add";
		break;
	case "edit_danhmuc":
		get_danhmuc();
		$template = "know/danhmuc_add";
...
function get_items(){
	...
	$sql_sp = "SELECT id,tinnoibat FROM table_know where id='".$id_up."' ";
	$d->query($sql_sp);
	...
	if($spdc1==0)
	{
	$sqlUPDATE_ORDER = "UPDATE table_know SET tinnoibat ='".$tinnoibat."' WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	}
	else
	{
	$sqlUPDATE_ORDER = "UPDATE table_know SET tinnoibat =0  WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	...
	$sql_sp = "SELECT id,hienthi FROM table_know where id='".$id_up."' ";
	$d->query($sql_sp);
	$danhmucs= $d->result_array();
	...
	if($hienthi==0)
	{
	$sqlUPDATE_ORDER = "UPDATE table_know SET hienthi =1 WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	}
	else
	{
	$sqlUPDATE_ORDER = "UPDATE table_know SET hienthi =0  WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	...
	$sql_sp = "SELECT id,noibat FROM table_know where id='".$id_up."' ";
	$d->query($sql_sp);
	$danhmucs= $d->result_array();
	...
	if($hienthi==0)
	{
	$sqlUPDATE_ORDER = "UPDATE table_know SET noibat =1 WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	}
	else
	{
	$sqlUPDATE_ORDER = "UPDATE table_know SET noibat =0  WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");

	}	
	}
	$sql = "select * from #_know";
...
function get_item(){
	...
	$sql = "select * from #_know where id='".$id."'";
	$d->query($sql);
...
function delete_item(){
	...
	if(isset($_GET['id'])){
		...
		if($d->num_rows()>0){
			...
			$sql = "delete from #_know where id='".$id."'";
			$d->query($sql);
		}
...
function get_danhmucs(){
	global $d, $items, $paging;
	$sql = "select * from #_know_danhmuc order by stt";
	$d->query($sql);
	$items = $d->result_array();
...
function get_danhmuc(){
	...
	$sql = "select * from #_know_danhmuc where id='".$id."'";
	$d->query($sql);
...
function delete_danhmuc(){
	...
	if(isset($_GET['id'])){
		$id =  themdau($_GET['id']);
		$d->reset();		
		$sql = "delete from #_know_danhmuc where id='".$id."'";
		if($d->query($sql)){
			$d->query("select * from #_know where id_danhmuc = ".$id);
			foreach($d->result_array() as $k=>$v){
				delete_file(_upload_tintuc.$v['photo']);
				delete_file(_upload_tintuc.$v['thumb']);
				$d->query("delete from #_know where id=".$v['id']);
			}
			redirect("index.php?com=".$_GET['com']."&act=man_danhmuc");
		}
		else
