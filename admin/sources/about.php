...
switch($act){
	...
	case "man_danhmuc":
		get_danhmucs();
		$template = "about/danhmucs";
		break;
	case "add_danhmuc":
		initAdd("about_danhmuc");
		$template = "about/danhmuc_add";
		break;
	case "edit_danhmuc":
		get_danhmuc();
		$template = "about/danhmuc_add";
...
function get_items(){
	...
	$sql_sp = "SELECT id,tinnoibat FROM table_about where id='".$id_up."' ";
	$d->query($sql_sp);
	...
	if($spdc1==0)
	{
	$sqlUPDATE_ORDER = "UPDATE table_about SET tinnoibat ='".$tinnoibat."' WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	}
	else
	{
	$sqlUPDATE_ORDER = "UPDATE table_about SET tinnoibat =0  WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	...
	if($hienthi==0)
	{
	$sqlUPDATE_ORDER = "UPDATE table_about SET noibat =1 WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	}
	else
	{
...
function get_danhmucs(){
	global $d, $items, $paging;
	$sql = "select * from #_about_danhmuc order by stt desc";
	$d->query($sql);
	$items = $d->result_array();
...
function get_danhmuc(){
	...
	$sql = "select * from #_about_danhmuc where id='".$id."'";
	$d->query($sql);
...
function delete_danhmuc(){
	...
	if(isset($_GET['id'])){
		$id =  themdau($_GET['id']);
		$d->reset();		
		$sql = "delete from #_about_danhmuc where id='".$id."'";
		if($d->query($sql)){
			$d->query("select * from #_about where id_danhmuc = ".$id);
			foreach($d->result_array() as $k=>$v){
				delete_file(_upload_tintuc.$v['photo']);
				delete_file(_upload_tintuc.$v['thumb']);
				$d->query("delete from #_about where id=".$v['id']);
			}
			redirect("index.php?com=".$_GET['com']."&act=man_danhmuc");
		}
		else
...
function get_lists(){
	global $d, $items, $paging;
	$sql = "select * from #_about_list order by stt desc";
	$d->query($sql);
	$items = $d->result_array();
...
function delete_list(){
	...
	if(isset($_GET['id'])){
		...
		if($d->query($sql)){
			$d->query("select * from #_about where id_list = ".$id);
			foreach($d->result_array() as $k=>$v){
				delete_file(_upload_tintuc.$v['photo']);
				delete_file(_upload_tintuc.$v['thumb']);
				$d->query("delete from #_about where id=".$v['id']);
