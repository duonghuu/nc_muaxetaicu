...
@define("_table","tailieu");
switch($act){
	case "rss":
		initAddRss();
		
		$template = "tailieu/item_add_rss";
		break;
	case "man":
		get_items();
	...
	case "man_cat":
		get_cats();
		$template = "tailieu/cats";
		break;
	case "add_cat":
		$template = "tailieu/cat_add";
		break;
	case "edit_cat":
		get_cat();
		$template = "tailieu/cat_add";
...
function get_items(){
	...
	$sql_sp = "SELECT id,tinnoibat FROM table_tailieu where id='".$id_up."' ";
	$d->query($sql_sp);
	$cats= $d->result_array();
	...
	if($spdc1==0)
	{
	$sqlUPDATE_ORDER = "UPDATE table_tailieu SET tinnoibat ='".$tinnoibat."' WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	}
	else
	{
	$sqlUPDATE_ORDER = "UPDATE table_tailieu SET tinnoibat =0  WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	...
	$sql_sp = "SELECT id,hienthi FROM table_tailieu where id='".$id_up."' ";
	$d->query($sql_sp);
	$cats= $d->result_array();
	...
	if($hienthi==0)
	{
	$sqlUPDATE_ORDER = "UPDATE table_tailieu SET hienthi =1 WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	}
	else
	{
	$sqlUPDATE_ORDER = "UPDATE table_tailieu SET hienthi =0  WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	...
	$sql_sp = "SELECT id,noibat FROM table_tailieu where id='".$id_up."' ";
	$d->query($sql_sp);
	$cats= $d->result_array();
	...
	if($hienthi==0)
	{
	$sqlUPDATE_ORDER = "UPDATE table_tailieu SET noibat =1 WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	}
	else
	{
	$sqlUPDATE_ORDER = "UPDATE table_tailieu SET noibat =0  WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");

	}	
	}
	$sql = "select * from #_tailieu";
...
function get_cats(){
	global $d, $items, $paging;
	$sql = "select * from #_tailieu_item order by stt";
	$d->query($sql);
	$items = $d->result_array();
...
function get_cat(){
	...
	$sql = "select * from #_tailieu_item where id='".$id."'";
	$d->query($sql);
...
function delete_cat(){
	...
	if(isset($_GET['id'])){
		$id =  themdau($_GET['id']);
		$d->reset();		
		$sql = "delete from #_tailieu_item where id='".$id."'";
		if($d->query($sql))
			redirect("index.php?com=".$_GET['com']."&act=man_cat");
		else
...
function initAddRss(){
	...
	if(isAjaxRequest()){
		...
		if($_GET['method'] == 'get-item'){
			...
			$img = $_POST['image'];
			$url = $_POST['url'];
			$d->query("select * from #_tailieu where ten_vi = '".$name."'");
			$data_ = array();
			if($d->num_rows() == 1){
				$data_['cls'] = 'red';
