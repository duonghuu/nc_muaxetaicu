...
@define("_table","aocuoi");
switch($act){
	case "rss":
		initAddRss();
		
		$template = "aocuoi/item_add_rss";
		break;
	case "man":
		get_items();
		
		$template = "aocuoi/items";
		break;
	case "add":
		initAdd();
		$template = "aocuoi/item_add";
		break;
	case "edit":	
		initAdd();		
		get_item();	
	...
	case "man_danhmuc":
		get_danhmucs();
		$template = "aocuoi/danhmucs";
		break;
	case "add_danhmuc":
		initAdd("aocuoi_danhmuc");
		$template = "aocuoi/danhmuc_add";
		break;
	case "edit_danhmuc":
		get_danhmuc();
		$template = "aocuoi/danhmuc_add";
	...
	case "man_list":
		get_lists();
		$template = "aocuoi/lists";
		break;
	case "add_list":
		initAdd("aocuoi_list");
		$template = "aocuoi/list_add";
		break;
	case "edit_list":
		get_list();
		$template = "aocuoi/list_add";
...
function get_items(){
	...
	$sql_sp = "SELECT id,tinnoibat FROM table_aocuoi where id='".$id_up."' ";
	$d->query($sql_sp);
	...
	if($spdc1==0)
	{
	$sqlUPDATE_ORDER = "UPDATE table_aocuoi SET tinnoibat ='".$tinnoibat."' WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	}
	else
	{
	$sqlUPDATE_ORDER = "UPDATE table_aocuoi SET tinnoibat =0  WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	...
	$sql_sp = "SELECT id,hienthi FROM table_aocuoi where id='".$id_up."' ";
	$d->query($sql_sp);
	$danhmucs= $d->result_array();
	...
	if($hienthi==0)
	{
	$sqlUPDATE_ORDER = "UPDATE table_aocuoi SET hienthi =1 WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	}
	else
	{
	$sqlUPDATE_ORDER = "UPDATE table_aocuoi SET hienthi =0  WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	...
	$sql_sp = "SELECT id,noibat FROM table_aocuoi where id='".$id_up."' ";
	$d->query($sql_sp);
	$danhmucs= $d->result_array();
	...
	if($hienthi==0)
	{
	$sqlUPDATE_ORDER = "UPDATE table_aocuoi SET noibat =1 WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	}
	else
	{
	$sqlUPDATE_ORDER = "UPDATE table_aocuoi SET noibat =0  WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");

	}	
	}
	$sql = "select * from #_aocuoi";
...
function get_item(){
	...
	$sql = "select * from #_aocuoi where id='".$id."'";
	$d->query($sql);
...
function delete_item(){
	...
	if(isset($_GET['id'])){
		...
		if($d->num_rows()>0){
			...
			$sql = "delete from #_aocuoi where id='".$id."'";
			$d->query($sql);
		}
...
function get_danhmucs(){
	global $d, $items, $paging;
	$sql = "select * from #_aocuoi_danhmuc order by stt desc";
	$d->query($sql);
	$items = $d->result_array();
...
function get_danhmuc(){
	...
	$sql = "select * from #_aocuoi_danhmuc where id='".$id."'";
	$d->query($sql);
...
function delete_danhmuc(){
	...
	if(isset($_GET['id'])){
		$id =  themdau($_GET['id']);
		$d->reset();		
		$sql = "delete from #_aocuoi_danhmuc where id='".$id."'";
		if($d->query($sql)){
			$d->query("select * from #_aocuoi where id_danhmuc = ".$id);
			foreach($d->result_array() as $k=>$v){
				delete_file(_upload_tintuc.$v['photo']);
				delete_file(_upload_tintuc.$v['thumb']);
				$d->query("delete from #_aocuoi where id=".$v['id']);
			}
			redirect("index.php?com=".$_GET['com']."&act=man_danhmuc");
		}
		else
...
function get_lists(){
	global $d, $items, $paging;
	$sql = "select * from #_aocuoi_list order by stt desc";
	$d->query($sql);
	$items = $d->result_array();
...
function get_list(){
	...
	$sql = "select * from #_aocuoi_list where id='".$id."'";
	$d->query($sql);
...
function delete_list(){
	...
	if(isset($_GET['id'])){
		$id =  themdau($_GET['id']);
		$d->reset();		
		$sql = "delete from #_aocuoi_list where id='".$id."'";
		if($d->query($sql)){
			$d->query("select * from #_aocuoi where id_list = ".$id);
			foreach($d->result_array() as $k=>$v){
				delete_file(_upload_tintuc.$v['photo']);
				delete_file(_upload_tintuc.$v['thumb']);
				$d->query("delete from #_aocuoi where id=".$v['id']);
