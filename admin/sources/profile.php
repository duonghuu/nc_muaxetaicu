...
@define("_table","profile");
switch($act){
	case "rss":
		initAddRss();
		
		$template = "profile/item_add_rss";
		break;
	case "man":
		get_items();
	...
	case "man_danhmuc":
		get_danhmucs();
		$template = "profile/danhmucs";
		break;
	case "add_danhmuc":
		initAdd("profile_danhmuc");
		$template = "profile/danhmuc_add";
		break;
	case "edit_danhmuc":
		get_danhmuc();
		$template = "profile/danhmuc_add";
	...
	case "man_list":
		...
		break;
	case "add_list":
		initAdd("profile_list");
		$template = "profile/list_add";
		break;
	case "edit_list":
		get_list();
		$template = "profile/list_add";
...
function get_items(){
	...
	$sql_sp = "SELECT id,tinnoibat FROM table_profile where id='".$id_up."' ";
	$d->query($sql_sp);
	...
	if($spdc1==0)
	{
	$sqlUPDATE_ORDER = "UPDATE table_profile SET tinnoibat ='".$tinnoibat."' WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	}
	else
	{
	$sqlUPDATE_ORDER = "UPDATE table_profile SET tinnoibat =0  WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	...
	$sql_sp = "SELECT id,hienthi FROM table_profile where id='".$id_up."' ";
	$d->query($sql_sp);
	$danhmucs= $d->result_array();
	...
	if($hienthi==0)
	{
	$sqlUPDATE_ORDER = "UPDATE table_profile SET hienthi =1 WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	}
	else
	{
	$sqlUPDATE_ORDER = "UPDATE table_profile SET hienthi =0  WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	...
	$sql_sp = "SELECT id,noibat FROM table_profile where id='".$id_up."' ";
	$d->query($sql_sp);
	$danhmucs= $d->result_array();
	...
	if($hienthi==0)
	{
	$sqlUPDATE_ORDER = "UPDATE table_profile SET noibat =1 WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	}
	else
	{
	$sqlUPDATE_ORDER = "UPDATE table_profile SET noibat =0  WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");

	}	
	}
	$sql = "select * from #_profile";
...
function get_item(){
	...
	$sql = "select * from #_profile where id='".$id."'";
	$d->query($sql);
...
function delete_item(){
	...
	if(isset($_GET['id'])){
		...
		if($d->num_rows()>0){
			...
			$sql = "delete from #_profile where id='".$id."'";
			$d->query($sql);
		}
...
function get_danhmucs(){
	global $d, $items, $paging;
	$sql = "select * from #_profile_danhmuc order by stt desc";
	$d->query($sql);
	$items = $d->result_array();
...
function get_danhmuc(){
	...
	$sql = "select * from #_profile_danhmuc where id='".$id."'";
	$d->query($sql);
...
function delete_danhmuc(){
	...
	if(isset($_GET['id'])){
		$id =  themdau($_GET['id']);
		$d->reset();		
		$sql = "delete from #_profile_danhmuc where id='".$id."'";
		if($d->query($sql)){
			$d->query("select * from #_profile where id_danhmuc = ".$id);
			foreach($d->result_array() as $k=>$v){
				delete_file(_upload_tintuc.$v['photo']);
				delete_file(_upload_tintuc.$v['thumb']);
				$d->query("delete from #_profile where id=".$v['id']);
			}
			redirect("index.php?com=".$_GET['com']."&act=man_danhmuc");
		}
		else
...
function get_lists(){
	global $d, $items, $paging;
	$sql = "select * from #_profile_list order by stt desc";
	$d->query($sql);
	$items = $d->result_array();
...
function get_list(){
	...
	$sql = "select * from #_profile_list where id='".$id."'";
	$d->query($sql);
...
function delete_list(){
	...
	if(isset($_GET['id'])){
		$id =  themdau($_GET['id']);
		$d->reset();		
		$sql = "delete from #_profile_list where id='".$id."'";
		if($d->query($sql)){
			$d->query("select * from #_profile where id_list = ".$id);
			foreach($d->result_array() as $k=>$v){
				delete_file(_upload_tintuc.$v['photo']);
				delete_file(_upload_tintuc.$v['thumb']);
				$d->query("delete from #_profile where id=".$v['id']);
