...
switch($act){
	...
	case "man_danhmuc":
		get_danhmucs();
		$template = "video/danhmucs";
		break;
	case "add_danhmuc":		
		initAdd("video_danhmuc");
		$template = "video/danhmuc_add";
		break;
	case "edit_danhmuc":		
		get_danhmuc();
		$template = "video/danhmuc_add";
...
function get_cats(){
	global $d, $items, $paging;
	$sql = "select * from #_video_item order by stt";
...
function initAddRss(){
	...
	if(isAjaxRequest()){
		...
		if($_GET['method'] == 'get-item'){
			...
			$img = $_POST['image'];
			$url = $_POST['url'];
			$d->query("select * from #_video where ten_vi = '".$name."'");
			$data_ = array();
			if($d->num_rows() == 1){
				$data_['cls'] = 'red';
...
function get_danhmucs(){
	...
	{
	$id_up = $_REQUEST['hienthi'];
	$sql_sp = "SELECT id,hienthi FROM table_video_danhmuc where id='".$id_up."' ";
	$d->query($sql_sp);
	$cats= $d->result_array();
	$hienthi=$cats[0]['hienthi'];
	if($hienthi==0)
	{
	$sqlUPDATE_ORDER = "UPDATE table_video_danhmuc SET hienthi =1 WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	}
	else
	{
	$sqlUPDATE_ORDER = "UPDATE table_video_danhmuc SET hienthi =0  WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	}	
	}
	
	$sql = "select * from #_video_danhmuc";
	...
	$curPage = isset($_GET['curPage']) ? $_GET['curPage'] : 1;
	$url="index.php?com=video&act=man_danhmuc";
	$maxR=20;
	$maxP=10;
	$paging=paging($items, $url, $curPage, $maxR, $maxP);
	$items=$paging['source'];
...
function get_danhmuc(){
	...
	$id = isset($_GET['id']) ? themdau($_GET['id']) : "";
	if(!$id)
	transfer("Không nhận được dữ liệu", "index.php?com=video&act=man_danhmuc");
	
	$sql = "select * from #_video_danhmuc where id='".$id."'";
	$d->query($sql);
	if($d->num_rows()==0) transfer("Dữ liệu không có thực", "index.php?com=video&act=man_danhmuc");
	$item = $d->fetch_array();	
}
...
function save_danhmuc(){
	...
	if(empty($_POST)) transfer("Không nhận được dữ liệu", "index.php?com=video&act=man_danhmuc");
	$id = isset($_POST['id']) ? themdau($_POST['id']) : "";
	$data = array();
	$data['stt'] = $_POST['stt'];
	$data['hienthi'] = isset($_POST['hienthi']) ? 1 : 0;
	...
	if($id){		
		...
		$d->setWhere('id', $id);
		if($d->update($data))
			redirect("index.php?com=video&act=man_danhmuc&curPage=".$_REQUEST['curPage']."");
		else
			transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=video&act=man_danhmuc");
	}else{			
		  if($photo = upload_image("file", 'jpg|png|gif|JPG|jpeg|JPEG|Jpg|PNG', _upload_tintuc,$file_name)){
			$data['photo'] = $photo;		
		...
		$d->setTable('video_danhmuc');
		if($d->insert($data))
			redirect("index.php?com=video&act=man_danhmuc");
		else
			transfer("Lưu dữ liệu bị lỗi", "index.php?com=video&act=man_danhmuc");
	}
}
...
function delete_danhmuc(){
	...
	if(isset($_GET['id'])){
		...
		$d->reset();		
			//Xóa danh mục cấp 1
			$sql = "delete from #_video_danhmuc where id='".$id."'";
			$d->query($sql);
			//Xóa danh mục cấp 2			
			$sql = "select id,thumb,photo from #_video_list where id_danhmuc='".$id."'";
			$d->query($sql);
			if($d->num_rows()>0)
			{
				while($row = $d->fetch_array())
				...
				$sql = "delete from #_video_list where id_danhmuc='".$id."'";
				$d->query($sql);
			}
			//Xóa danh mục cấp 3
			$sql = "select id,thumb,photo from #_video_cat where id_danhmuc='".$id."'";
			$d->query($sql);
			if($d->num_rows()>0)
			{
				while($row = $d->fetch_array())
				...
				$sql = "delete from #_video_cat where id='".$id."'";
				$d->query($sql);
			}
			//Xóa danh mục cấp 4			
			$sql = "select id,thumb,photo from #_video_item where id_danhmuc='".$id."'";
			$d->query($sql);
			if($d->num_rows()>0)
			{
				while($row = $d->fetch_array())
				...
				$sql = "delete from #_video_item where id_danhmuc='".$id."'";
				$d->query($sql);
			}
			//Xóa sản phẩm thuộc loại đó			
			$sql = "select id,thumb,photo from #_video where id_danhmuc='".$id."'";
			$d->query($sql);
			if($d->num_rows()>0)
			{
				while($row = $d->fetch_array())
				...
				$sql = "delete from #_video where id_danhmuc='".$id."'";
				$d->query($sql);
			}
		...
		if($d->query($sql))
			...
			transfer("Xóa dữ liệu bị lỗi", "index.php?com=video&act=man_danhmuc");
	}
	elseif (isset($_GET['listid'])==true)
	{
		$listid = explode(",",$_GET['listid']); 
		for ($i=0 ; $i<count($listid) ; $i++){
			...
			$d->reset();
				...
				$sql = "delete from #_video_cat where id_danhmuc='".$id."'";
				$d->query($sql);
		...
		} redirect("index.php?com=video&act=man_danhmuc");} else transfer("Không nhận được dữ liệu", "index.php?com=video&act=man_danhmuc"	    );
}
