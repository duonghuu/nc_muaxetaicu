...
@define("_table","service");
switch($act){
	case "rss":
		initAddRss();
		
		$template = "service/item_add_rss";
		break;
	case "man":
		get_items();
	...
	case "man_danhmuc":
		get_danhmucs();
		$template = "service/danhmucs";
		break;
	case "add_danhmuc":
		initAdd("service_danhmuc");
		$template = "service/danhmuc_add";
		break;
	case "edit_danhmuc":
		get_danhmuc();
		$template = "service/danhmuc_add";
...
function get_items(){
	...
	$sql_sp = "SELECT id,tinnoibat FROM table_service where id='".$id_up."' ";
	$d->query($sql_sp);
	...
	if($spdc1==0)
	{
	$sqlUPDATE_ORDER = "UPDATE table_service SET tinnoibat ='".$tinnoibat."' WHERE  id = ".$id_up."";
	...
	if($hienthi==0)
	{
	$sqlUPDATE_ORDER = "UPDATE table_service SET noibat =1 WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	}
	else
	{
...
function get_danhmucs(){
	global $d, $items, $paging;
	$sql = "select * from #_service_danhmuc order by stt";
	$d->query($sql);
	$items = $d->result_array();
...
function get_danhmuc(){
	...
	$sql = "select * from #_service_danhmuc where id='".$id."'";
	$d->query($sql);
...
function delete_danhmuc(){
	...
	if(isset($_GET['id'])){
		$id =  themdau($_GET['id']);
		$d->reset();		
		$sql = "delete from #_service_danhmuc where id='".$id."'";
		if($d->query($sql)){
			$d->query("select * from #_service where id_danhmuc = ".$id);
			foreach($d->result_array() as $k=>$v){
				delete_file(_upload_tintuc.$v['photo']);
				delete_file(_upload_tintuc.$v['thumb']);
				$d->query("delete from #_service where id=".$v['id']);
			}
			redirect("index.php?com=".$_GET['com']."&act=man_danhmuc");
		}
		else
