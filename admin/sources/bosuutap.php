...
@define("_table","bosuutap");
switch($act){
	case "rss":
		initAddRss();
		
		$template = "bosuutap/item_add_rss";
		break;
	case "man":
		get_items();
	...
	case "man_danhmuc":
		get_danhmucs();
		$template = "bosuutap/danhmucs";
		break;
	case "add_danhmuc":
		initAdd("bosuutap_danhmuc");
		$template = "bosuutap/danhmuc_add";
		break;
	case "edit_danhmuc":
		get_danhmuc();
		$template = "bosuutap/danhmuc_add";
...
function get_items(){
	...
	$sql_sp = "SELECT id,noibat FROM table_bosuutap where id='".$id_up."' ";
	$d->query($sql_sp);
	$danhmucs= $d->result_array();
	...
	if($hienthi==0)
	{
	$sqlUPDATE_ORDER = "UPDATE table_bosuutap SET noibat =1 WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	}
	else
	{
	$sqlUPDATE_ORDER = "UPDATE table_bosuutap SET noibat =0  WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
...
function get_danhmucs(){
	global $d, $items, $paging;
	$sql = "select * from #_bosuutap_danhmuc order by stt";
	$d->query($sql);
	$items = $d->result_array();
...
function get_danhmuc(){
	...
	$sql = "select * from #_bosuutap_danhmuc where id='".$id."'";
	$d->query($sql);
...
function delete_danhmuc(){
	...
	if(isset($_GET['id'])){
		$id =  themdau($_GET['id']);
		$d->reset();		
		$sql = "delete from #_bosuutap_danhmuc where id='".$id."'";
		if($d->query($sql)){
			$d->query("select * from #_bosuutap where id_danhmuc = ".$id);
			foreach($d->result_array() as $k=>$v){
				delete_file(_upload_tintuc.$v['photo']);
				delete_file(_upload_tintuc.$v['thumb']);
				$d->query("delete from #_bosuutap where id=".$v['id']);
			}
			redirect("index.php?com=".$_GET['com']."&act=man_danhmuc");
		}
		else
